...
const document: TypedDocumentNode<QueryPlaygroundResponse, OperationVariables> = gql`query Playground($slug: String!) { playground(slug: $slug) { success message error data { id slug title description icon kind steps { id sortIndex title body commandHint actionHint } } } }`
...
export const queryPlayground = async ({ slug, headers, signal, debug }: QueryPlaygroundParams) =>
    createApolloClient({ headers, signal, debug }).query({ query: document, variables: { slug } })
