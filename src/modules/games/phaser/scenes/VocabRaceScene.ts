...
export class VocabRaceScene extends BaseGameScene {
    ...
    protected positionPlayers(snapshot: GameSnapshot): void {
        const track = GAME_WIDTH - 480
        const mia = snapshot.players.find((p) => p.character === "MIA")
        const max = snapshot.players.find((p) => p.character === "MAX")
        if (this.mia && mia) {
            this.mia.x = 220 + Math.min(100, mia.progress) / 100 * track
        }
        if (this.max && max) {
