...
export const CoursePlaygroundSessionPage = ({ displayId, slug }: CoursePlaygroundSessionPageProps) => {
    ...
    const resolveState = (): CoursePlaygroundSessionState => {
        if (session.failed || session.startFailed || session.session === null || session.socketState === "failed") return "failed"
        if (completed) return "completed"
        if (session.socketState === "reconnecting") return "reconnecting"
        if (session.socketState !== "connected" || !session.agentConnected) return "connecting"
        return "live"
    }
    const state = resolveState()
    const connectionTextByState: Record<CoursePlaygroundSessionState, string> = {
        live: t("session.agentConnected"),
        reconnecting: t("session.reconnecting"),
        completed: t("session.completed"),
        failed: t("session.waiting"),
        connecting: t("session.waiting"),
    }
    const connectionText = connectionTextByState[state]

    return (
        <CoursePlaygroundSessionPageView
            state={state}
            props={{
