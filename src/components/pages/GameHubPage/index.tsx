...
export const GameHubPage = () => {
    ...
    const chooseCatalog = useCallback(() => document.querySelector("[data-node=game-catalog-section]")?.scrollIntoView({ behavior: "smooth", block: "start" }), [])
    ...
    return <><_GameHubPage standingSurface={Standing} catalogSurface={Catalog} runnerSurface={active === undefined ? undefined : Runner} /><GameSetupOverlay game={selected} isOpen={selected !== undefined} onResolved={resolve} onCancelled={() => setSelected(undefined)} /><SignInOverlay isOpen={signIn} onDismiss={() => setSignIn(false)} /></>
}
/** Declares the game hub orchestrator as a connected page. */
export const meta = { shape: "page", world: "connected", domain: "games" } as const
