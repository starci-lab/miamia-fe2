...
describe("ProfilePublicCvPageBase", () => {
    ...
    it("distinguishes no file from uncompiled without drawing an iframe", () => {
        const { rerender, container } = render(<ProfilePublicCvPageBase state="empty" props={{ label: "Public CV", message: "No public CV", title: "CV", editLabel: "Edit CV", retryLabel: "Retry", isSelf: false }} />)
        expect(screen.getByText("No public CV")).toBeInTheDocument()
        expect(container.querySelector("iframe")).toBeNull()
        rerender(<ProfilePublicCvPageBase state="uncompiled" props={{ label: "Public CV", message: "Not compiled", title: "CV", editLabel: "Edit CV", retryLabel: "Retry", isSelf: false }} />)
