...
vi.mock("./component", () => ({ _CourseLeaderboardPage: ({ state, on }: TestPageInput) => <><output data-testid="state">{state}</output><button onClick={on.course}>course</button><button onClick={() => on.selectCategory("challenge")}>category</button><button onClick={() => on.selectCategory("reading")}>reading</button><button onClick={() => on.selectCategory("unknown")}>unknown</button><button onClick={on.climb}>climb</button><button onClick={on.retry}>retry</button></> }))
