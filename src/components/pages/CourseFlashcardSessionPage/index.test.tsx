...
vi.mock("./component", () => ({ _CourseFlashcardSessionPage: ({ state, on }: TestPageInput) => <><output data-testid="state">{state}</output><button onClick={on.reveal}>reveal</button><button onClick={() => on.rate(2)}>rate</button><button onClick={() => on.answerQuiz?.(false)}>answer</button><button onClick={on.retry}>retry</button><button onClick={on.leave}>leave</button></> }))
