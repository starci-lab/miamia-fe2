...
export const ProfileChallengeSubmissionPageBase = ({ state, detail, onBack }: ProfileChallengeSubmissionPageProps) => {
    ...
    return <Grammar contract="profile-main" render={createGrammarNode("profile-main", { section: [
        createGrammarProjection("label-row-over-card", () => <Grammar contract="profile-proof-summary" render={createGrammarNode("profile-proof-summary", {
            back: createLeafNode("button", {}, () => <Button props={{ label: `← ${detail?.courseTitle ?? "Challenges"}`, variant: "ghost", size: "sm" }} on={{ press: onBack }} />),
            title: createLeafNode("heading", {}, () => <Heading props={{ content: resolveSummaryTitle(state, missing, detail?.title), level: 2 }} isLoading={state === "pending"} />),
            meta: createLeafNode("text", { size: "sm", tone: "muted" }, () => <Text props={{ content: missing ? "This submission is not public." : [detail?.difficulty, detail?.selectedLang, detail?.score == null ? undefined : `score ${detail.score}`, detail?.passedAt].filter(Boolean).join(" · "), size: "sm", tone: "muted" }} isLoading={state === "pending"} />),
        })} />),
        ...(detail?.submissionUrl || state === "pending" ? [createGrammarProjection("label-row-over-card", () => <SurfaceCard props={{ label: "Submitted proof" }} contract="profile-meta-list" render={createGrammarNode("profile-meta-list", {
            item: [createLeafNode("link", {}, () => <Link props={{ label: detail?.submissionUrl ?? "Loading proof", externalHref: detail?.submissionUrl ?? undefined }} isLoading={state === "pending"} />)],
        })} />)] : []),
        createGrammarProjection("label-row-over-card", () => <SurfaceCard props={{ label: "Attempts" }} contract="profile-evidence-list" render={createGrammarNode("profile-evidence-list", {
            evidence: attempts.length > 0 ? attempts.map((attempt, index) => createCompositeNode("evidence-row", {}, () => <EvidenceRow props={{ title: `Attempt ${attempt.attemptNumber ?? index + 1}${index === 0 && (attempt.score ?? 0) > 0 ? " · Passed" : ""}`, subtitle: [attempt.processedAt, attempt.shortFeedback].filter(Boolean).join(" · "), fact: attempt.score == null ? undefined : String(attempt.score), factTone: (attempt.score ?? 0) >= 80 ? "success" : "neutral" }} isLoading={state === "pending"} />)) : [createCompositeNode("evidence-row", {}, () => <EvidenceRow props={{ title: "No public attempts were found." }} />)],
        })} />),
        createGrammarProjection("label-row-over-card", () => <SurfaceCard props={{ label: "Structured feedback" }} contract="profile-evidence-list" render={createGrammarNode("profile-evidence-list", {
            evidence: feedbacks.length > 0 ? feedbacks.map((feedback, index) => createCompositeNode("evidence-row", {}, () => <EvidenceRow props={{ title: feedback.message ?? `Feedback ${index + 1}`, subtitle: feedback.detail ?? feedback.suggestion, fact: feedback.severity, factTone: feedback.severity?.toLowerCase().includes("strong") ? "success" : "warning" }} isLoading={state === "pending"} />)) : [createCompositeNode("evidence-row", {}, () => <EvidenceRow props={{ title: "No structured feedback was published." }} />)],
        })} />),
    ] })} />
}
