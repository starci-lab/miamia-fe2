...
export const UpcomingLivestreamRow = ({ props, on, isLoading = false }: CompositeProps<UpcomingLivestreamRowData, UpcomingLivestreamRowActions>) => {
    const body = createGrammarNode("evidence-title-over-subtitle", {
        title: createLeafNode("text", { size: "sm", weight: "semibold" }, () => <Text props={{ content: props.title, size: "sm", weight: "semibold" }} isLoading={isLoading} />),
        ...(props.subtitle === undefined ? {} : { subtitle: createLeafNode("text", { size: "xs", tone: "muted" }, () => <Text props={{ content: props.subtitle, size: "xs", tone: "muted" }} />) }),
    })
    const content = createGrammarNode("upcoming-livestream-row", {
        mark: createLeafNode("icon-tile", {}, () => <IconTile props={{ icon: "livestream", tone: "accent", size: "md" }} isLoading={isLoading} />), body,
        time: createLeafNode("text", { size: "xs", tone: "muted" }, () => <Text props={{ content: props.time, size: "xs", tone: "muted" }} isLoading={isLoading} />),
    })
