...
export const GameLobbyPanel = ({ props, on }: CompositeProps<GameLobbyPanelData, GameLobbyPanelActions>) => <SurfaceCard contract="game-lobby-card" render={defineContractComponent("game-lobby-card", {
    title: defineLeafComponent("heading", {}, () => <Heading props={{ content: props.title, level: 2 }} />),
    ...(props.roomCode === undefined ? {} : { code: defineLeafComponent("text", { size: "md", weight: "semibold" }, () => <Text props={{ content: `Mã phòng: ${props.roomCode}`, weight: "semibold" }} />) }), // vn-ok: localized runtime copy
    ...
    ...(props.roomCode === undefined ? {} : { action: defineLeafComponent("button", {}, () => <Button props={{ label: props.copyLabel, variant: "outline", size: "sm" }} on={{ press: on?.copyCode }} />) }),
})} />
