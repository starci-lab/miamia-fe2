...
export const ProfileRow = ({ props, on, isLoading = false }: ProfileRowProps) => {
    const identity = layoutNode("profile-name-over-handle", {
        name: renderLeaf("text", { size: "sm", weight: "semibold" }, () => <Text props={{ content: props.displayName, size: "sm", weight: "semibold" }} isLoading={isLoading} />),
        handle: renderLeaf("text", { size: "xs", tone: "muted" }, () => <Text props={{ content: props.username === undefined ? undefined : `@${props.username}`, size: "xs" }} isLoading={isLoading} />),
    })
    const content = layoutNode("profile-avatar-name-handle-disclosure-row", {
        avatar: renderLeaf("avatar", {}, () => <Avatar props={{ name: props.displayName, src: props.avatar, size: "md" }} isLoading={isLoading} />),
        identity,
        disclosure: renderLeaf("icon", {}, () => <Icon props={{ name: "disclosure", role: "chip" }} />),
    })
    return <PressableSurface layout="profile-avatar-name-handle-disclosure-row" render={content} label={props.displayName ?? "Profile"} press={on?.press} />
}
