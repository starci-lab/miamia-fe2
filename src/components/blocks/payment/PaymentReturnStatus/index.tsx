...
export const PaymentReturnStatus = ({ referenceId, onContinue }: PaymentReturnStatusConnectedProps) => {
    ...
    if (status.error) return <PaymentReturnStatusBase badge={t("failedBadge")} title={t("failedTitle")} body={t("failedBody")} tone="danger" action={t("retry")} onAction={() => void status.mutate()} />
    const value = status.data?.status ?? "pending"
    if (value === "pending") return <PaymentReturnStatusBase badge={t("pendingBadge")} title={t("pendingTitle")} body={t("pendingBody")} tone="warning" action={t("checkAgain")} onAction={() => void status.mutate()} />
    if (value === "succeeded") return <PaymentReturnStatusBase badge={t("successBadge")} title={t("successTitle")} body={t(status.data?.purchaseKind === "membership" ? "successMembership" : "successDownload")} tone="success" action={t("continue")} onAction={onContinue} />
