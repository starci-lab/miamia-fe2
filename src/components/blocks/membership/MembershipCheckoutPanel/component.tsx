...
/** Defines the stateful layout for the pure checkout panel. */
...
export const MembershipCheckoutPanelBase = (input: MembershipCheckoutPanelProps) => (
    <Grammar layout="purchase-checkout-panel" render={layoutNode("purchase-checkout-panel", {
        ...
        price: renderLeaf("text", {}, () => <Text props={{ content: input.props.price, weight: "semibold" }} isLoading={input.state === "loading"} />),
        benefit: input.props.benefits.map((benefit) => renderLeaf("text", {}, () => <Text props={{ content: benefit, icon: "complete" }} />)),
        ...(input.state === "failed" ? {
            notice: renderComposite("empty-notice", {}, () => <EmptyNotice props={{ message: input.props.errorMessage, actionLabel: input.props.checkoutLabel }} on={{ act: input.on?.retry }} />),
        } : {}),
        action: [
            renderLeaf("button", {}, () => <Button props={{ label: input.props.checkoutLabel, variant: "primary", isPending: input.state === "submitting", disabled: input.state === "failed" || input.state === "loading" }} on={{ press: input.on?.checkout }} />),
