...
describe("StudyTopicCatalogBase", () => { it("opens the selected real topic", () => { vi.stubGlobal("ResizeObserver", class { observe() {} unobserve() {} disconnect() {} }); const open = vi.fn(); render(<StudyTopicCatalogBase state="ready" props={props} on={{ "open:1": open }} />); fireEvent.click(screen.getByRole("button", { name: "Xem" })); expect(open).toHaveBeenCalledOnce() }) })
