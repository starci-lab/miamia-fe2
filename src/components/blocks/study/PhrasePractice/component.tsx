...
export const _PhrasePractice = (input: PhrasePracticeProps) => {
    ...
    return <Tree contract="study-practice-stack" render={defineContractComponent("study-practice-stack", {
        ...
        ...(terminal ? { notice: defineCompositeComponent("empty-notice", {}, () => <EmptyNotice props={{ icon: "practice", message: input.state === "pending" ? input.props.loading : input.state === "failed" ? input.props.failed : input.props.empty, actionLabel: input.state === "failed" ? input.props.retry : undefined }} on={{ act: input.on?.retry }} />) } : isResult ? {
