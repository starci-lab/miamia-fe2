...
export const UpcomingLivestreamCard = () => {
    ...
    const rows = [...(query.data ?? [])]
        ...
        .map((item) => ({
            ...
            time: new Date(item.nextStartAt).toLocaleString(locale, { dateStyle: "medium", timeStyle: "short" }),
    ...
    const openRow = async (routeId: string) => {
        setPending(routeId)
        try {
            const result = await route.trigger({ globalId: routeId })
