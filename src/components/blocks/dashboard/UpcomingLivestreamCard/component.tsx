...
const View = ({ props, on, isLoading = false }: ComponentProps<UpcomingData, UpcomingActions>) => { const rows = isLoading ? Array.from({ length: COUNT }, (_, i) => ({ id: `resting-${i}` })) : props.rows; return <Grammar contract="upcoming-livestream-list" render={createGrammarNode("upcoming-livestream-list", { session: rows.map((row) => createCompositeNode("upcoming-livestream-row", {}, () => <UpcomingLivestreamRow props={row} on={{ open: on?.[`open:${row.id}`] }} isLoading={isLoading} />)) })} /> }
const List = createGrammarNode("upcoming-livestream-list", View)
...
export const UpcomingLivestreamCardBase = (input: UpcomingProps) => {
    ...
    return <SurfaceListCard contract="upcoming-livestream-list" render={List} props={input.props} on={input.on} isLoading={input.state === "pending"} />
}
