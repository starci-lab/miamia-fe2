...
vi.mock("./component", () => ({ _TopLearners: ({ state, on }: LearnerProps) => <><output data-testid="state">{state}</output><button onClick={on?.retry}>retry</button><button onClick={on?.seeMore}>more</button></> }))
