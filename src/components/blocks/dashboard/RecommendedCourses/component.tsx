...
const COUNT = LAYOUTS["recommended-course-list"].children.course.restingCount
const View = ({ props, on, isLoading = false }: ComponentProps<RecommendedCoursesData, RecommendedCoursesActions>) => { const rows = isLoading ? Array.from({ length: COUNT }, (_, i) => ({ id: `resting-${i}` })) : props.rows; return <Grammar layout="recommended-course-list" render={layoutNode("recommended-course-list", { course: rows.map((row) => renderComposite("recommended-course-row", {}, () => <RecommendedCourseRow props={row} on={{ open: on?.[`open:${row.id}`], openPriceDetail: on?.[`priceDetail:${row.id}`] }} isLoading={isLoading} />)) })} /> }
const List = layoutNode("recommended-course-list", View)
...
export const RecommendedCoursesBase = (input: RecommendedCoursesProps) => {
    ...
    return <SurfaceListCard layout="recommended-course-list" render={List} props={input.props} on={input.on} isLoading={input.state === "pending"} />
}
