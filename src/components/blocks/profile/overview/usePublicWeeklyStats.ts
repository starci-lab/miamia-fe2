...
type WeeklyStatsVariables = { readonly userId: string }
const document: TypedDocumentNode<WeeklyStatsResponse, WeeklyStatsVariables> = gql`query ProfileWeeklyStats($userId: ID!) { userWeeklyStats(userId: $userId) { success message error data { streak longestStreak } } }`
...
export const usePublicWeeklyStats = () => {
    ...
    const request = useSWR<WeeklyStats>(userId ? ["QUERY_PROFILE_WEEKLY_STATS", userId] : null, async () => {
        const result = await createApolloClient({ withAuth: true }).query({ query: document, variables: { userId: userId ?? "" }, fetchPolicy: "no-cache" })
